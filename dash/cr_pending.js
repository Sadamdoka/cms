...
document.getElementById('para_active').addEventListener('click', crApprove);
function crApprove() {
    var genid = document.getElementById("con_genid").value;
    var remark = document.getElementById("para_msg").value;
    let  status = $("#para_co_status :selected").attr('id');

    fetch(url + "conditions/contract/" + genid + "/" + status,
            {
                method: 'GET'
            }).then(function (response) {
        console.log('Response: ' + response.status);
        if (response.status === 200) {
            logger(genid, "Contract Renwal", remark,act_name);
            alert("Successfull");
