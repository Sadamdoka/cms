                ...
                try {
                    ...
                    if (!isEmpty(data)) {
                        ...
                        if (!isJsonArray(jdata)) {
                            ...
                            loadJO(jdata.organ, 'det_fco', "<option  disabled selected hidden >Choose Job Order</option>");
                ...
                try {
                    ...
                    if (!isEmpty(data)) {
                        ...
                        if (!isJsonArray(jdata)) {
                            ...
                            e_data += '<option id="' + jdata.joid + '" value="' + jdata.id + '" name="' + jdata.category + '" ">';
...
function getJODetails_Table(input) {
    let id = $(input).attr("id");
    //console.log(input);

    try {
        $.ajax({
            url: url + "fetch/jo/0/" + id,
            dataType: 'json',
            type: 'get',
            cache: false,
            processData: false,
...
//
            ...
            contentType: false,
            beforeSend: function () {               //tbody.html("<tr><td colspan='5' align='center'><i class = 'fa fa-spinner spin'></i> Loading</td></tr>");
                $("#jo_body").html('<tr><td colspan="20" align="center"><div class="spinner-border text-primary" role="status"><span class="sr-only">Loading...</span></div></td></tr>');
            },
            complete: function (data) {
                //tbody.html("<i class = 'fa fa-spinner spin'></i> Please Wait.."+ JSON.stringify(data));
            },
            success: function (data) {
                ...
                try {
                    ...
                    if (!isEmpty(data)) {
                        row += "";
                        var value = data.job_order;
                        if (!isJsonArray(value)) {

                            e_data += '<tr>';
                            e_data += '<td>';
                            e_data += '<button id="' + value.joid + '" onclick="getJODetails_Table(this)" value = "' + value.id + '"  type="button"  class="btn btn-info" >View</button>';
                            e_data += '</td>';
                            e_data += '<td>' + getDate_formart(value.datereg) + '</td>';
                            e_data += '<td>' + value.joid + '</td>';
                            e_data += '<td>' + value.name + '</td>';
                            e_data += '<td>' + value.category + '</td>';
                            e_data += '<td>' + value.gender + '</td>';
                            e_data += '<td>' + value.qty + '</td>';
                            e_data += '<td>' + value.salary + '</td>';
                            e_data += '<td>' + value.period + '</td>';
                            e_data += '<td>' + getJoStatus(value.status) + '</td>';
                            e_data += '</tr>';
                        } else {
                            $.each(data.job_order, function (index, value) {
                                e_data += '<tr>';
...
function getJoStatus(input) {
    if (input === '0') {
        return '<span class="badge badge-warning">Pending</span>';
    } else if (input === '1') {
        return '<span class="badge badge-dark">Paid</span>';
    } else if (input === '2') {
